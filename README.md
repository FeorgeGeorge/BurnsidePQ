...
The road stops here. Burnside's theorem is the goal of this development, and the results above are
what Isaacs' Chapters 2 to 7 contribute to it; nothing beyond — the Feit–Thompson odd
order theorem, Bender–Suzuki, the classification proper — is attempted. Within the chain itself,
Chapter 7's other main results are proved as well: Frobenius' normal `p`-complement theorem 5.26
(`Isaacs/Frobenius.lean`) and Thompson's normal `p`-complement theorem 7.1
(`Isaacs/ThompsonNormalPComplement.lean`), which together complete the proof that Frobenius
kernels are nilpotent.
